...
def make_output_dir(output_dir: Path, events):
    ...
    for event in events:
        ...
        event_dir_path: Path = output_dir / "{}_{}_{}".format(event.pandda_name, event.dtag, event.event_idx)
        ...
        try:
            ...
            already_built.append("{}_{}_{}".format(event.pandda_name, event.dtag, event.event_idx))
...
class AutobuildWrapper:
    def __init__(self,
        ...
        self.autobuild_func = partial(autobuild_func,
                                      *args,
                                      **kwargs,
                                      )
    ...
    def __call__(self):
        print("Checking if already ran...")
        if self.output_path.is_file():
            return None

        print("Running autobuild func...")
        result: Result = self.autobuild_func()

        print("Converting result to series...")
        # results_series = pd.Series({"time": result.time,
        #                             "result_model_paths": result.result_model_paths,
        #                             "success": result.success, })
        # result_array = np.array([str(result.time), str(result.success)] + [str(model_path)
        #                                                                    for model_path
        #                                                                    in result.result_model_paths
        #                                                                    ]
        #                         )
        result_json = json.dumps({"time": str(result.time),
                                    "result_model_paths": [str(result_path)
                                                           for result_path
                                                           in result.result_model_paths],
                                    "success": str(result.success) })

        print("Outputting result series...")
        # np.save(str(self.output_path),
        #         result_array,
        #         allow_pickle=False,
        #         )
        # results_series.to_csv(str(self.output_path))
...
if __name__ == "__main__":
    ...
    for event in events:
        ...
        try:
            ...
            event_output_dir_path: Path = config.output_dir / "{}_{}_{}".format(event.pandda_name,
                                                                                event.dtag,
                                                                             event.event_idx,
                                                                             )
            ...
            placed_ligand_path = event_output_dir_path / "ligand.pdb"
            if not (event_output_dir_path / "ligand.pdb").is_file():
            ...
            stripped_receptor_path = event_output_dir_path / "stripped_receptor.pdb"
            if not (event_output_dir_path / "stripped_receptor.pdb").is_file():
            ...
            event_map_mtz_path = event_output_dir_path / "{}_{}.mtz".format(event.dtag, event.event_idx)
            if not (event_output_dir_path / "{}_{}.mtz".format(event.dtag, event.event_idx)).is_file():
            ...
            # Phenix
            # Phenix control
            phenix_control_output_path = event_output_dir_path / "phenix_control.json"
            phenix_control = AutobuildWrapper(autobuild_event_phenix,
                                              phenix_control_output_path,
                                              protein_model_path=stripped_receptor_path,
                                              ligand_model_path=ligand_model_path,
                                              event_mtz_path=data_path,
                                              output_dir_path=event_output_dir_path / "phenix_control",
                                              event_centroid=None,
                                              )
            output_paths[(event.dtag, event.event_idx, "phenix_control")] = phenix_control_output_path
            tasks[(event.dtag, event.event_idx, "phenix_control")] = phenix_control

            # Phenix location
            # Phenix Event map
            phenix_event_output_path = event_output_dir_path / "phenix_event.json"
            phenix_event = AutobuildWrapper(autobuild_event_phenix,
                                            phenix_event_output_path,
                                            protein_model_path=stripped_receptor_path,
            ...
            output_paths[(event.dtag, event.event_idx, "phenix_event")] = phenix_event_output_path
            tasks[(event.dtag, event.event_idx, "phenix_event")] = phenix_event
    ...
    print("Preparing to autobuild: {} tasks".format(len(tasks.values())))
    # list(tasks.values())[0]()
    # exit()

    # processor = ProcessorLuigi(jobs=2,
    #                            parallel_env="smp",
    #                            n_cpu=3,
    #                            run_locally=False,
    #                            h_vmem=20,
    ...
    #                            )
    #
    # processor(tasks.values(),
    #           output_paths.values(),
    #           )

    results = process_dask(list(tasks.values()),
                           jobs=125,
                           cores=1,
                           processes=1,
                           h_vmem=15,
    ...
    event_build_tables = {}
    for event_build_id, event_build_table_path in output_paths.items():
        event_build_tables[event_build_id] = pd.read_csv(str(event_build_table_path))
