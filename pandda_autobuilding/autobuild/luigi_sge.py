...
def _parse_qsub_job_id(qsub_out):
    ...
    job_id = re.findall(regex, str(qsub_out))[0]
    print(job_id)
    return int(job_id)
...
class SGEJobTask(luigi.Task):
    ...
    def _run_job(self):
        ...
        job_str = '/dls/science/groups/i04-1/conor_dev/anaconda/envs/env_clipper_no_mkl/bin/python {0} "{1}" "{2}"'.format(
    ...
    def _track_job(self):
        while True:
            ...
            print("QSTAT OUT: {}".format(qstat_out))
            qstat_out = str(qstat_out)
            sge_status = _parse_qstat_state(str(qstat_out), self.job_id)
            if sge_status == 'r':
